...
namespace LoLAData {
   ...
   local::stream_protocol::socket &getSocket() {
      ...
      if (!isConnected) {
         ...
         isConnected = !ec;
         if (ec) {
            llog(ERROR) << "******** Could not connect to LoLA: " << ec.message() << std::endl;
   ...
   const map<string, vector<msgpack::object> > read() {
      ...
      isConnected = !ec;
      if (!ec) {
         msgpack::object_handle objectHandle = msgpack::unpack(data, MSGPACK_READ_LENGTH);
         msgpack::object        obj          = objectHandle.get();
         return obj.as<map<string, vector<msgpack::object> > >();
      } else {
         llog(ERROR) << "******** Could not read from LoLA: " << ec.message() << std::endl;
         SAY("Could not read from LoLA: " + ec.message());
         return map<string, vector<msgpack::object> >();
      }
   }
   ...
   bool          head_limp    = false, limp = true, standing = false, sitUnstiff = true;
