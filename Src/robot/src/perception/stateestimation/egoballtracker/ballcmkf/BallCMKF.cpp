    ...
    EstimatorInfoOut &estimatorInfoOut)
{
    ...
    AbsCoord ballPos = getBallAbsCoord();
    estimatorInfoOut.ballPosRRC = ballPos;

    AbsCoord ballVel = getBallVelAbsCoord();
    estimatorInfoOut.ballVelRRC = ballVel;

    estimatorInfoOut.sharedStateEstimationBundle.ballPosRRC = ballPos;
    estimatorInfoOut.sharedStateEstimationBundle.ballVelRRC = ballVel;
    ...
    RRCoord ballPosRR = ballPos.convertToRobotRelative();
    AbsCoord ballPosWorld = AbsCoord(
        robotPos.x() + ballPosRR.distance() * cosf(robotPos.theta() + ballPosRR.heading()),
        robotPos.y() + ballPosRR.distance() * sinf(robotPos.theta() + ballPosRR.heading()),
        0);
    // clip ball back on to field
    ballPosWorld.x() = std::min((FIELD_LENGTH + POSSIBLE_OFF_FIELD_BALL_MARGIN) / 2.0f,
                          std::max(- (FIELD_LENGTH + POSSIBLE_OFF_FIELD_BALL_MARGIN) / 2.0f, ballPosWorld.x()));
    ballPosWorld.y() = std::min((FIELD_WIDTH + POSSIBLE_OFF_FIELD_BALL_MARGIN) / 2.0f,
                          std::max(- (FIELD_WIDTH + POSSIBLE_OFF_FIELD_BALL_MARGIN) / 2.0f, ballPosWorld.y()));
    ...
    RRCoord ballVelRR = ballVel.convertToRobotRelative();
...
AbsCoord BallCMKF::getBallAbsCoord()
{
...
AbsCoord BallCMKF::getBallVelAbsCoord()
{
