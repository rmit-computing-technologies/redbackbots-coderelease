...
RRCoord CameraToRR::convertToRR(const Point &p, const CameraInfo& cameraInfo, bool isBall) const {
   return convertToRR(p.x (), p.y (), cameraInfo, isBall);
}

RRCoord CameraToRR::convertToRR(int16_t i, int16_t j, const CameraInfo& cameraInfo, bool isBall) const {
   RRCoord myloc = pose.imageToRobotRelative(i, j, cameraInfo, isBall ? BALL_RADIUS : 0);
   return myloc;
}

Point CameraToRR::convertToRRXY(const Point &p) const {
   Point myloc = pose.imageToRobotXY_old(p, 0);
   return myloc;
}
...
float CameraToRR::pixelSeparationToDistance(int pixelSeparation, int realSeparation) const {
   return (DEFAULT_FOCAL_LENGTH * realSeparation) / (TOP_PIXEL_SIZE * pixelSeparation);
}
