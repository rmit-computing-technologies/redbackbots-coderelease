...
// TODO: CHECK THIS CHANGE OF HEADER FILE
   ...
   switch (io) {
      ...
      case IO_METHOD_USERPTR:
         ...
         for (i = 0; i < n_buffers; ++i) {
            // TODO: Check type of uint64_t is ok, over uint32_t
            if (lastDequeued.m.userptr == (uint64_t) buffers[i].start
...
const uint8_t *NaoCamera::get(const __u32 colourSpace) {
   ...
   if (io == IO_METHOD_MMAP) {
      ...
      // If we hear this on a V6, that's an issue.
      if (0 == r) {
...
bool NaoCamera::init_camera() {
   if (v4lDeviceP) {
	...
	// }
      ...
      // TODO: These are alvision constants and not used in V4L - replace
      switch (format) {
         case k960p:
   ...
   switch (io) {
      ...
      case IO_METHOD_USERPTR:
         for (i = 0; i < n_buffers; ++i) {
            ...
            // TODO: CHeck use of uint64_t is ok here
            buf.type        = V4L2_BUF_TYPE_VIDEO_CAPTURE;
