                                        ...
                                        bool top)
{
   ...
   if (lines.size() == 2) {
       ...
       */
      llog(DEBUG) << "sanity check 1: intersect inside frame" << endl;
      ...
      if (lines.size() == 2) {
          ...
          */
         llog(DEBUG) << "sanity check 2: angle between lines" << endl;
         ...
         {
            lines.pop_back();
            llog(DEBUG) << "Failed sanity check 2" << std::endl;
            llog(DEBUG) << "m1: " << m1 << ", m2: " << m2 << ", theta: " << RAD2DEG(theta) << std::endl;
         }
      }
   }
