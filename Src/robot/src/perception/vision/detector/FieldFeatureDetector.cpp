...
void FieldFeatureDetector::detect(const VisionInfoIn& info_in, const VisionInfoMiddle& info_middle, VisionInfoOut& info_out) {
    ...
    llog(TRACE) << "Finding field feature candidate points took " <<
    ...
    field_features_.insert(field_features_.end(),
                         ...
                         temp_features_.end());
    llog(TRACE) << "Finding basic field features took " << t.elapsed_us() <<
                                                             " us" << std::endl;
    ...
    llog(TRACE) << "Counting lines and circles took " << t.elapsed_us();
    llog(TRACE) << " us" << std::endl;
    ...
    if (num_circles == 0) {
        ...
        //llog(TRACE) << "Looking for parallel lines took " << t.elapsed_us();
        //llog(TRACE) << " us" << std::endl;
    } else if (num_circles == 1 && num_lines == 1) {
        circleOrientation(&field_features_);
        llog(TRACE) << "Circle orientation took " << t.elapsed_us();
        llog(TRACE) << " us" << std::endl;
    }
    ...
    // }
    llog(TRACE) << "Penalty spot detection took " << t.elapsed_us();
    llog(TRACE) << " us" << std::endl;
