...
#define CIRCLE_FIT_MAX_CENTER_POINTS 300
...
BallDetector::BallDetector() : estimator(ball) {
    nn = load_3_layer_cnn();
}
...
void BallDetector::regenerateRegion(BallDetectorVisionBundle &bdvb, bool aspectCheck){
    ...
    RegionI *region = new RegionI(*bdvb.region, BBox(newBounds.a, newBounds.b), 1, 0, true);
...
// We don't want to early exit while using vdm
        ...
        // We don't want to early exit while using vdm
                    ...
                    if (vdm != NULL) {
                        VisionDebugQuery q = vdm->getQuery();
                        cout << "REGION " << region_index << "/" << q.region_index << endl;
                        cout << "SUBREGION " << subregion_index_blob << "/" << q.subregion_index << endl;
                        if (region_index == q.region_index && subregion_index_blob == q.subregion_index) {
                            itb->drawBall();
                        }
                    }
#endif // BALL_DETECTOR_USES_VDM
...
bool BallDetector::inspectBall(BallDetectorVisionBundle &bdvb){
    ...
    RegionI *modelRegion = new RegionI(*bdvb.region, 1, 0, true);
...
#ifdef BALL_TO_FILE_COMP
    char name[] = "a";
    char location[] = ball_dir;
    std::stringstream dir;
    dir << location << ball_count << ".png";
    ball_count++;
    WriteImage w;
    if (w.writeImage(*bdvb.modelRegion, COLOUR_FORMAT, dir.str().c_str(), name)){
        std::cout << "Success\n";
    }
    else{
        std::cout << "Fail\n";
	...
	// #ifdef CTC_2_1
	// if (estimator.predict(*bdvb.modelRegion) == CLASSIFIER_FALSE || dnn_predict(resized, nn) == 0) {
	// if (estimator.predict(*bdvb.modelRegion) == CLASSIFIER_FALSE) {
	if (dnn_predict(resized, nn) == 0) {
#ifdef BALL_DEBUG
		cout << "GMM classified FALSE\n";
#endif  // BALL_DEBUG
		return false;
	}
	// TODO: need to test if DNN code below is needed. Can it be used as a fallback for GMM or as well as?
	// #else
	// Eigen::MatrixXf imageMat = convertMat(*bdvb.modelRegion);
	// Eigen::MatrixXf resized = dnn_resize(imageMat, 32, 32);
...
void BallDetector::rescaleRegion(BallDetectorVisionBundle &bdvb) {
    ...
    const RegionI *new_region = new RegionI((*bdvb.region), 1 << density_change, DENSITY_DECREASE, true);
...
void BallDetector::preProcessAdaptive(BallDetectorVisionBundle &bdvb) {
    ...
    RegionI *region = new RegionI(*bdvb.region, 1, 0, true, bdvb.window_size, bdvb.percentage);
...
bool BallDetector::analyseInternalRegions(InternalRegionFeatures &internal_region_features, BallDetectorVisionBundle &bdvb) {
	...
	return internal_region_features.num_internal_regions >= 1 && internal_region_features.max_internal_region_prop >= 0.04;
}
