...
PythonSkill::PythonSkill(Blackboard *bb) : Adapter(bb) {
   ...
   if (blackboard->config["debug.file_watcher"].as<bool>()) {
       startInotify();
   }
   startPython();
}
...
void PythonSkill::startInotify() {
    ...
    registerListener(std::string(path) + "body/roles");
    registerListener(std::string(path) + "util");
...
BehaviourRequest PythonSkill::execute() {
   ...
   try {
      bool reloadNeeded = inotify_fd != -1 && inotify_Check();
