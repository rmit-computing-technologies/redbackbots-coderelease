...
public:
  ...
  RingBuffer(std::size_t capacity = n) :
    buffer(reinterpret_cast<T*>(alignedMalloc(capacity * sizeof(T)))),
    allocated(capacity)
  {}
  ...
  RingBuffer(const RingBuffer& other) :
    buffer(reinterpret_cast<T*>(alignedMalloc(other.allocated * sizeof(T)))),
   ...
   */
  T& operator[](std::size_t index) { return buffer[(allocated + head - index - 1) % allocated];}
  const T& operator[](std::size_t index) const { return buffer[(allocated + head - index - 1) % allocated];}
  ...
  T& back() { return (*this)[entries - 1];}
  const T& back() const { return (*this)[entries - 1];}
  ...
  void reserve(std::size_t capacity)
  {
    if(capacity != allocated)
    {
      ...
      if(prev)
        free(reinterpret_cast<char*>(prev));
    }
  }
