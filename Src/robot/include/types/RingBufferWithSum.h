...
public:
  ...
  {
    static_assert(!std::is_floating_point<T>::value && !std::is_integral<T>::value, "Use the standard constructor instead!");
    clear();
  }
  ...
  */
  ...
  inline T getMAD()
  {
    ...
    T median = copy[(int) (RingBuffer<T, n>::size() / 2)];
    for (T& t : copy)
    {
        t = std::abs(median - t);
    }
    ...
    return copy[(int) (RingBuffer<T, n>::size() / 2)];
  }
};
