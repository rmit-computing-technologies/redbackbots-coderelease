...
class ScanLineMiddleInfo : public Detector {
   ...
   */
	...
	*/
	...
	/**
		...
		* @param ecImage ECImage passed through middle_info
		* @param scanGrid ScanGrid passed through middle_info
	*/
  	void scanHorizontal(unsigned int y, std::vector<InternalRegion>& regions, const unsigned int leftmostX, const unsigned int rightmostX, 
  	...
  	/**
		...
		* @param scanGrid ScanGrid passed through middle_info
   	*/
  	void scanHorizontalAdditionalSmoothing(unsigned int y, std::vector<InternalRegion>& regions, const unsigned int leftmostX, const unsigned int rightmostX,
		ECImage& ecImage, ScanGrid& scanGrid) const;
	...
	/**
		...
		* @param scanGrid ScanGrid passed through info_middle
		* @param ecImage ECImage passed through info_middle
	*/
  	void scanVertical(const ScanGrid::ScanGridLine& line, int middle, int top, std::vector<InternalRegion>& regions,
   	...
   	*/
  	template <int filterSize>
  	void findEdgeInSubLineHorizontal(std::vector<InternalRegion>& regions, ScanRun<filterSize>& scanRun, unsigned int startPos, unsigned int stopPos, bool maxEdge,
		ECImage& ecImage) const;
	...
	*/
  	template <int filterSize>
  	void findEdgeInSubLineVertical(std::vector<InternalRegion>& regions, ScanRun<filterSize>& scanRun, unsigned int startPos, unsigned int stopPos, bool maxEdge,
		ECImage& ecImage) const;
	...
	*/
  	...
  	/**
		...
		* @param scanGrid ScanGrid passed through info_middle
		* @param relativeFieldColorsParameters RelativeFieldColorsParameters passed through info_middle
   	*/
  	...
  	/**
		...
		* @param scanGrid ScanGrid passed through info_middle
   	*/
  	void classifyFieldHorizontal(const std::vector<unsigned short>& y, std::vector<std::vector<InternalRegion>>& regions,
  	...
  	/**
		...
		* @param scanGrid ScanGrid passed through info_middle
   	*/
  	void classifyFieldVertical(std::vector<std::vector<InternalRegion>>& regions,
		const CameraInfo& cameraInfo, ScanGrid& scanGrid) const;
	 ...
	 * @param horizontal Are the regions on horizontal (true) or vertical (false) scan lines
	 */
	void classifyFieldSingleRegion(InternalRegion& region, float regionHeight, bool horizontal, const CameraInfo& cameraInfo, ScanGrid& scanGrid) const;
	 ...
	 */
	unsigned char fieldClassificationSaturationThreshold(const InternalRegion& region, float regionHeight, const CameraInfo& cameraInfo, ScanGrid& scanGrid) const;
   	...
   	*/
  	[[nodiscard]] bool regionIsField(const InternalRegion* const region,
  	...
  	/**
		...
		* @param relativeFieldColorsParameters RelativeFieldColorsParameters passed through info_middle
   	*/
  	void classifyWhiteRegionsWithThreshold(std::vector<std::vector<InternalRegion>>& regions,
	...
	*/
	...
	/**
		...
		* @param cameraImage CameraImage passes through info_in
		* @return True, if the EstimatedFieldColor is valid.
	*/
  	[[nodiscard]] bool isEstimatedFieldColorValid(const CameraImage* cameraImage) const;
	...
	*/
  	...
  	/**
		...
		* @param relativeFieldColors RelativeFieldColors passed from info_middle
   	*/
  	void approximateBaseLuminance(RelativeFieldColors& relativeFieldColors);

  	/**
   		* Approximates the images average saturation.
		* @param relativeFieldColors RelativeFieldColors passed from info_middle
   	*/
  	void approximateBaseSaturation(RelativeFieldColors& relativeFieldColors);


public:

	ScanLineMiddleInfo(Blackboard* blackboard);
  	virtual ~ScanLineMiddleInfo();

	// Resets middle info
	virtual void resetMiddleInfo(VisionInfoMiddle* info_middle);
	virtual void resetVisionOut(VisionInfoOut* info_out);

	Vector2f additionalSmoothingPoint = {1500.f, 0.f};  /**< On field distance in mm up to which additional smoothing is applied */
	float edgeThreshold = 13;                   		/**< The edge threshold. */
	unsigned char luminanceSimilarityThreshold = 20;  	/**< Maximum luminance difference for two regions to become united */
	unsigned char hueSimilarityThreshold = 20;        	/**< Maximum hue difference for two regions to become united */
	unsigned char saturationSimilarityThreshold = 26; 	/**< Maximum saturation difference for two regions to become united */
	unsigned char initialMinSaturation = 138;          /**< Standard minimum field saturation for field classification */
	int lowerMinRegionSize = 520;                 		/**< Minimal size in covered scanline pixels for initial field regions on the lower camera */
	int upperMinRegionSize = 640;                 		/**< Minimal size in covered scanline pixels for initial field regions on the upper camera */
	float baseLuminanceReduction = 0.9f;             	/**< Rather underestimate the baseLuminance as it is used for noise filtering */
	short verticalGridScanMinStep = 2;              	/**< Minimum distance between two vertical scan steps */
	bool prelabelAsWhite = true;                    	/**< Label upper regions that are brighter than both neighbors as white during region building */
	short maxPrelabelRegionSize = 20;               	/**< Maximum region size to prelabel as white */
	short maxRegionSizeForStitching = 12;          		/**< Maximum size in pixels of a none region between field and white or field and field for stitching */
	int estimatedFieldColorInvalidationTime = 400;     	/**< Time in ms until the EstimatedFieldColor is invalidated */
...
private:
	...
	void updateRegionsHorizontal(ColorScanLineRegionsHorizontal& colorScanLineRegionsHorizontal, 
								CameraInfo::Camera whichCamera, 
								const VisionInfoIn* info_in, 
								VisionInfoMiddle* info_middle, 
								VisionInfoOut* info_out);

	void updateRegionsVerticalClipped(ColorScanLineRegionsVerticalClipped& colorScanLineRegionsVerticalClipped, 
								CameraInfo::Camera whichCamera, 
								const VisionInfoIn* info_in, 
								VisionInfoMiddle* info_middle, 
								VisionInfoOut* info_out);
