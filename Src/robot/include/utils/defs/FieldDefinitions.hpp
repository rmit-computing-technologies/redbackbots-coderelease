...
/** Maximum possible distance across a field (field diagonal including the border)*/
DECLARE_CUSTOM_CONST(MAX_DIST_ON_FIELD, 12674, 8522);
...
/** Penalty Cross - i.e. dimensions of square fitted around it */
// this is 100 for competition and for our small field
constexpr float PENALTY_CROSS_DIMENSIONS = 100;

/* Penalty Cross - to middle of the closest penalty cross (official 1300) */
DECLARE_CUSTOM_CONST(DIST_GOAL_LINE_TO_PENALTY_CROSS, 1300, 806); 

/* Penalty Cross - position relative to centre of field */
constexpr float PENALTY_CROSS_ABS_X = (FIELD_LENGTH / 2 - DIST_GOAL_LINE_TO_PENALTY_CROSS);
...
DECLARE_CUSTOM_CONST(GOAL_DEPTH, 500, 500); // Measured from the front edge of the crossbar to the centre of the rear bar
...
/** Packet limit per team */
constexpr int PACKET_LIMIT = 1200;

/** Game time excluding initial to ready time (seconds) */
constexpr int GAME_TIME = 1200;

/** Maximum message size as per the SPL rules (in bytes) */
constexpr std::size_t MAX_MESSAGE_SIZE = 128;