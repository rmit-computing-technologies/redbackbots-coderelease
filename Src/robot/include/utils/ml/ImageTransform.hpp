...
{
  template<typename T>
  inline static __m128 setR2float(T, T, T, T);
  template<> inline __m128 setR2float(unsigned char v0, unsigned char v1, unsigned char v2, unsigned char v3)
  {
    return _mm_cvtepi32_ps(_mm_setr_epi32(v0, v1, v2, v3));
  }
  template<> inline __m128 setR2float(float v0, float v1, float v2, float v3)
  {
    return _mm_setr_ps(v0, v1, v2, v3);
  }

  template<typename T>
  inline static __m128 getPixel(const Image<T>& src, __m128 x_x_x_x_Dash, __m128 y_y_y_y_Dash, float defaultValue = 0.f)
  {
