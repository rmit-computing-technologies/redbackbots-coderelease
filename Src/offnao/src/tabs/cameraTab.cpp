   ...
   if (topFrame && botFrame) {
      ...
      for (unsigned int row = 0; row < imageRows; ++row) {
         for (unsigned int col = 0; col < imageCols; ++col) {
            ...
            pointCloud.points.push_back(make_pair(qglviewer::Vec(
                                                   r / 255.0,
                                                   g / 255.0,
                                                   b / 255.0),
                                                  qglviewer::Vec(
                                                   gety(frame, row, col, imageCols) / 255.0,
                                                   getu(frame, row, col, imageCols) / 255.0,
                                                   getv(frame, row, col, imageCols) / 255.0)));
         }
      }
...
void CameraTab::setControls(const vector<struct v4l2_control> &controls){
    ...
    Q_EMIT sendCommandToRobot(QString(ss.str().c_str()));
}
