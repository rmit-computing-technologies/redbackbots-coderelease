...
class Defender(Role):
    ...
    def default_position(self, robot_pos, ball_pos):
        ...
        offset_heading = normalisedTheta(
            OUR_GOAL_CENTRE.headingTo(ball_pos) +
            radians(-90) if self.in_left_side else radians(90))

        offset_vec = \
            makeVector2DFromDistHeading(offset_distance, offset_heading)
    ...
    def calculate_close_and_far_point_to_goal(
        ...
        dist_close_point = \
            clamp(goal_to_ball_dist - max_opponent_kick_distance,
                  close_point_min_dist_to_goal,
                  close_point_max_dist_to_goal)

        dist_far_point = \
            clamp(goal_to_ball_dist - max_opponent_kick_distance,
                  far_point_min_dist_to_goal,
                  far_point_max_dist_to_goal)

        close_point = OUR_GOAL_CENTRE.plus(
            makeVector2DFromDistHeading(
                dist_close_point, goal_to_ball_heading))
        far_point = OUR_GOAL_CENTRE.plus(
            makeVector2DFromDistHeading(
                dist_far_point, goal_to_ball_heading))
    ...
    def update_near_our_goal_box(self, ball_pos):
        if self.near_our_goal_box:
            if not isInOurGoalBox(ball_pos, buffx=700, buffy=700):
                self.near_our_goal_box = False
        else:
            if isInOurGoalBox(ball_pos, buffx=500, buffy=500):
    ...
    def update_in_our_corner(self, ball_pos):
        goal_to_ball_heading_abs = \
            abs(OUR_GOAL_CENTRE.headingTo(ball_pos))
...
class Supporter(Role):
    ...
    def default_position(self, robot_pos, ball_pos):
        ...
        if goal_to_ball_dist < goal_to_ball_dist_to_use_min:
            ...
            m = ((dist_to_ball_max - dist_to_ball_min) /
                 (goal_to_ball_dist_to_use_max - goal_to_ball_dist_to_use_min))
            b = -m * goal_to_ball_dist_to_use_min + dist_to_ball_min
            dist_to_ball = m * goal_to_ball_dist + b

        ball_to_our_goal_heading = \
            OUR_GOAL_CENTRE.minus(ball_pos).heading()

        position = ball_pos.plus(
            makeVector2DFromDistHeading(
                dist_to_ball, ball_to_our_goal_heading))
    ...
    def our_corner_position(self):
        if self.goalie_is_in_goal_box:
            ...
            # defender if there are already two robots in the goal box, and
            # also to avoid standing in the way of the goalie
            return Vector2D(-FIELD_LENGTH / 2 + GOAL_BOX_LENGTH + 300,
                            400 if self.in_left_side else -400)
        else:
            # position near our right goal post and block the goal
            post = OUR_LEFT_POST if self.in_left_side else OUR_RIGHT_POST
            return post.plus(
                Vector2D(250, -200 if self.in_left_side else 200))
    ...
    def our_goal_free_kick_position(self, ball_pos):
        # Stand sort of near the ball
        ball_from_supporter_vector = \
            Vector2D(500, 1000 if self.in_left_side else -1000)
    ...
    def enemy_goal_free_kick_position(self, ball_pos):
        # Stand in a blocking position, slightly offset
        # to not overlap with ballplayer
        ball_from_supporter_vector = \
            Vector2D(1600, 300 if self.in_left_side else -300)
        position = ball_pos.minus(ball_from_supporter_vector)
        X_MAX_ABS = FIELD_LENGTH / 2 - 400
        position.x = clamp(position.x, -X_MAX_ABS, X_MAX_ABS)
        return position
    ...
    def enemy_pushing_free_kick_position(self, ball_pos):
        ...
        # to not overlap with ballplayer
        ball_from_supporter_vector = \
            Vector2D(1600, 500 if self.in_left_side else -500)
        ball_from_supporter_vector.rotate(
            ball_pos.minus(OUR_GOAL_CENTRE).heading())
        position = ball_pos.minus(ball_from_supporter_vector)
        X_MAX_ABS = FIELD_LENGTH / 2 - 400
        position.x = clamp(position.x, -X_MAX_ABS, X_MAX_ABS)
        return position
    ...
    def enemy_kick_in_position(self, ball_pos):
        if self.in_left_side:
            ...
            ball_from_supporter_vector = \
                Vector2D(2000, 0).rotate(heading)
        else:
...
class Upfielder(Role):
    ...
    def default_position(self, robot_pos, ball_pos):
        ...
        # Calculate closest point from me to segment robot has to be on
        P, Q = \
            self.calculate_close_and_far_point_to_post(post, ball_pos=ball_pos)
        point_on_segment = closest_point_on_segment(robot_pos, P, Q)
    ...
    def calculate_close_and_far_point_to_post(
        ...
        max_close_point_dist = ball_to_post_dist - \
            close_point_min_distance_to_post
        max_far_point_dist = ball_to_post_dist - \
            far_point_min_distance_to_post

        dist_ball_to_close_point = min(
            max_close_point_dist,
            close_point_default_dist_to_ball)
        dist_ball_to_far_point = min(
            max_far_point_dist,
            far_point_default_dist_to_ball)

        close_point = ball_pos.plus(
            makeVector2DFromDistHeading(
                dist_ball_to_close_point, ball_to_post_heading))
        far_point = ball_pos.plus(
            makeVector2DFromDistHeading(
                dist_ball_to_far_point, ball_to_post_heading))
    ...
    def update_inside_enemy_goal_box(self, ball_pos):
        if self.inside_enemy_goal_box:
            if not isInOpponentGoalBox(ball_pos, buffx=100, buffy=100):
                self.inside_enemy_goal_box = False
        else:
            if isInOpponentGoalBox(ball_pos, buffx=-100, buffy=-100):
                self.inside_enemy_goal_box = True
    ...
    def update_in_enemy_corner(self, ball_pos):
        ...
        if self.in_enemy_corner:
            ...
            if ball_to_goal_heading_abs > radians(60):
...
class PositioningAgainstKickingTeam(Positioning):
    ...
    def __init__(self):
        ...
        # anticipating
        self.roles = [
            ["supporter", POSITIONING_AGAINST_KICKING_TEAM_SUPPORTER,
             Supporter(self)],
            ["defender", POSITIONING_AGAINST_KICKING_TEAM_DEFENDER,
             Defender(self)],
            ["upfielder", POSITIONING_AGAINST_KICKING_TEAM_UPFIELDER,
             Upfielder(self)]
        ]
    ...
    def evaluate(self):
        ...
        for player in get_active_player_numbers():
            if player is my_player_number():
                ...
                anticipators_current_positions.append(
                    get_teammate_pos(player))
        ...
        possible_roles = \
            self.roles[
                num_excessive_ball_players:
                num_roles + num_excessive_ball_players]
        ...
        time_to_reach_poses = [[10000000 for _ in range(num_anticipators)]
        ...
        for permutation in permutations:
            ...
            for anticipator_index, role_index in enumerate(permutation):
                times.append(
                    time_to_reach_poses[anticipator_index][role_index])
        ...
        best_role_index = \
            all_times_and_my_role[0][1] + num_excessive_ball_players
