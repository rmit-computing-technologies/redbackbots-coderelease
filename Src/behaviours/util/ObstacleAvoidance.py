...
def calculate_tangent_point(
        ...
        left_side=True):  # Whether we are considering the left tangent point, seen from the robot  # noqa
    ...
    centre_to_robot_radius = \
        Vector2D(radius, 0).rotated(robot_to_centre.multiply(-1).heading())

    # Radius-length vector from centre towards tangent point
    centre_to_t_p = \
        centre_to_robot_radius.rotated(-alpha if left_side else alpha)
...
def walk_vec_with_avoidance(walk_vec):
    ...
    if lfoot_bumper_interferes_walk:
        if 0 <= angle_lfoot_bumper_obs_to_walk_vec_heading < radians(90):
            return walk_vector_forwards.rotated(
                lfoot_bumper_obs_heading + radians(90))
    ...
    if rfoot_bumper_interferes_walk:
        if 0 <= angle_rfoot_bumper_obs_to_walk_vec_heading < radians(90):
            return walk_vector_forwards.rotated(
                rfoot_bumper_obs_heading + radians(90))
    ...
    if sonar_left_obstacle(clear_seconds=SONAR_CLEAR_MINIMUM_SECONDS):
        angle_lsonar_obs_to_walk_vec_heading = angleSignedDiff(
            walk_vec_heading, lsonar_obs_heading)
    ...
    if sonar_right_obstacle(clear_seconds=SONAR_CLEAR_MINIMUM_SECONDS):
        angle_rsonar_obs_to_walk_vec_heading = angleSignedDiff(
            walk_vec_heading, rsonar_obs_heading)
