...
def angleDiff(angle0, angle1):
    return min(math.fabs(angle0 - angle1),
               math.fabs(math.fabs(angle0 - angle1) - 2.0 * math.pi))
...
def absToRr(fromPos, toPos):
    ...
    heading = normalisedTheta(math.atan2(
        toPos[1]-fromPos[1], toPos[0]-fromPos[0])-fromPos[2])
    return (dist, heading)
