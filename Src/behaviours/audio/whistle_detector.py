...
3.  robot$ python whistle_detector.py
    NOTE: The whistle_detector.py will exit if no redbackbots process is running.
    Update check_redbackbots_running() function with your team's main executable.
4.  Say something.
5.  robot$ <Ctrl-C>
...
def check_on_nao():
    ...
    """
    try:
        cmd = subprocess.Popen(["whoami"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    except OSError:
        return False
    data = str(cmd.communicate()[0])
    lines = data.split('\n')
    has_nao = [l for l in lines if 'nao' in l]
    return has_nao

ON_NAO_ROBOT = check_on_nao()
# Set verbosity to between 0-3 to print less or more information
VERBOSITY = 1 if ON_NAO_ROBOT else 1
...
WHISTLE_FILE_FORMAT = "whistle_%J.wav"

# Function to convert datetime to Julian date
def datetime_to_julian_date(dt):
    hour_frac = dt.hour / 24.0
    minute_frac = dt.minute / (24.0 * 60.0)
    second_frac = dt.second / (24.0 * 60.0 * 60.0)
    day_frac = hour_frac + minute_frac + second_frac

    julian_date = 367 * dt.year - (7 * (dt.year + ((dt.month + 9) // 12))) // 4 + (
            (275 * dt.month) // 9) + dt.day + 1721013.5 + day_frac
...
class Config(object):
    ...
    def __init__(self):
        ...
        self.spectrum_threshold = 2.7 # amplitude std deviations (reduce to stop quiet whistles being heard)
        self.nWhistleOkaySpectra = 11  # ~234ms at 48 kHz -> To reduce false whistles make as high as possible
        self.nWhistleMissSpectra = 4  # ~83ms at 48 kHz -> Make as high as possible
...
def check_redbackbots_running():
    cmd1 = subprocess.Popen(["ps", "ax"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    data = str(cmd1.communicate()[0])
    lines = data.split("\n")
    has_redbackbots = [line for line in lines if "redbackbots" in line]
    if len(has_redbackbots) == 0:
        print("whistle_detector: exiting as redbackbots not running")
        exit(0)
    return True
...
class WhistleState(object):
    ...
    def interrogate(self, complex_spectrum):
        ...
        for _ in range(num_buckets):
            bucket_mean = numpy.mean(spectrum[round(begin) : round(begin) + round(grow_size)])
            if bucket_mean < background_growth_threshold:
                begin += grow_size
            else:
                break
        for _ in range(num_buckets):
            bucket_mean = numpy.mean(spectrum[round(end) - round(grow_size) : round(end)])
            if bucket_mean < background_growth_threshold:
                end -= grow_size
            else:
                break
        ...
        filtered = spectrum[round(begin):round(end)]
        found = False
        if filtered:
    ...
    class SoundReceiverModule(SoundReceiverMixin):
        ...
        def save_whistle_to_file(self, num_channels, time_stamp):
            ...
            if NAO_SAVE_WHISTLE:
                ...
                # new_file_name = dt.strftime(f"whistle_{julian_date:.0f}.wav")
                new_file_name = "Whistle_Has_Been_Heard.wav"
...
def pc_wav_test():
    ...
    default = os.path.join(os.environ["HOME"], "~/git/redbackbots")
    redbackbots_dir = os.environ.get("REDBACKBOTS_CHECKOUT_DIR", default)
    yes_dir = os.path.join(redbackbots_dir, "test/audio/whistle_yes")
    ...
    print("-" * 80)
    no_dir = os.path.join(redbackbots_dir, "test/audio/whistle_no")
...
if __name__ == "__main__":
    ...
    while ON_NAO_ROBOT and RETRY > 0:
        ...
        try:
            ...
            write_to_file_with_stack_trace("Crash caught, attempting to kill " "pulseaudio to see if it fixes it.")
