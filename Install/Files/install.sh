...
## + RedBackBots modifications WORK IN PROGRESS
##
...
create_user_partition() {
  ...
  watchdog $((15*60)) true "sync ; sync"
  watchdog $((1*60)) true "mkfs.ext2 -L RBB-data '$USER_PART'"
  watchdog $((15*60)) true "sync ; sync"
  killall -q -USR1 setears
}
...
setup_system() {
  ...
  if [ -r /tmp/userpart/nao/config/Robots/robotsv2.cfg ]; then
    local headId="$(/tmp/get-head-id)"
    if [ -n "${headId}" ]; then
      local thisRobot="$(grep "${headId}" /tmp/userpart/nao/.config/idmapping)"
      if [ -n "${thisRobot}" ]; then
        # hostname="$(echo ${thisRobot} | sed "s%.*name[ ]*=[ ]*\"\([^\"]*\)\";.*%\1%")"
        hostname="$(echo ${thisRobot} | sed "s%\(.*\)=.*%\1%")"
        if [ -n "${hostname}" ]; then
          local networkFile="/tmp/userpart/nao/config/Robots/${hostname}/robot.cfg"
          if [ -r "${networkFile}" ]; then
            wirelessIp="$(grep "^playerIP" "${networkFile}" | sed "s%^.*=\(.*\)$%\1%")"
            wiredIp=${wirelessIp}
          fi
        fi
      fi
  ...
  mkdir -p /tmp/userpart/nao/.ssh
  chmod 700 /tmp/userpart/nao/.ssh

  # Configure SSH Config
  sed -i '/UseDNS/s/.*/UseDNS no/'  /tmp/rootpart/etc/ssh/sshd_config
  sed -i '/PermitRootLogin/s/no/yes/' /tmp/rootpart/etc/ssh/sshd_config
